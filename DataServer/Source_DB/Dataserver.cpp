...
unsigned short g_DSPort = 0;
unsigned short g_DSPosition = 0;
unsigned int g_DSConnected = 0;
...
BOOL CDataserverApp::InitInstance()
{
	...
	m_pMainWnd = pFrame;
#if(NEW_WIN)
	pFrame->LoadFrame(IDR_MAINFRAME, WS_OVERLAPPED+WS_CAPTION+WS_SYSMENU+WS_MINIMIZEBOX+WS_VISIBLE, NULL, NULL);
#else
...
void CDataserverApp::Init()
{
	...
	g_DSPort = m_ServerPort;
	g_DSPosition = m_position;
