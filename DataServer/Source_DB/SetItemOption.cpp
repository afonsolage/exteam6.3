...
int CSetItemOption::IsSetItem(int itemnum)	
{
	...
	{
		LogAddC(LOGC_RED, "IsSetItem Check error: (itemnum:%d)", itemnum);
		return 0;
	}
...
int CSetItemOption::GetOptionTableIndex(int itemnum, int setoption)	
{
	if (itemnum<0 || itemnum > MAX_ITEM-1)
	{
		LogAddC(LOGC_RED, "GetOptionTableIndex Check error: (itemnum:%d)", itemnum);
		return -1;
	}

	if (setoption != 1 && setoption != 2)
	{
		LogAddC(LOGC_RED, "GetOptionTableIndex setoption Check error: (itemnum:%d)", itemnum);
		return -1;
	}
...
char * CSetItemOption::GetSetOptionName(int optionindex)
{
	...
	{
		LogAddC(LOGC_RED, "GetSetOptionName Check error: (itemnum:%d)", optionindex);
		return "";
	}
...
char * CSetItemOption::GetSetOptionName(int itemnum, int settype)
{
	...
	if (itemnum<0 || itemnum > MAX_ITEM-1)
	{
		LogAddC(LOGC_RED, "GetSetOptionName Check error: (itemnum:%d)", itemnum);
		return "";
	}
...
int CSetItemOption::GetSetOption(int optionindex, int optablenum, int& op1, int& op2, int& opvalue1,int& opvalue2, int Class, int ChangeUp)
{
	...
	if (optionindex <0 || optionindex > 79)
	{
		LogAddC(LOGC_RED, "GetSetOption Check error: (itemnum:%d)", optionindex);
		return 0;
	}
...
int CSetItemOption::GetMaxSetOptionCount(int optionindex)
{
	if (optionindex <0 || optionindex > 79)
	{
		LogAddC(LOGC_RED, "GetMaxOptionCount Check error: (itemnum:%d)", optionindex);
		return -1;
	}
...
int CSetItemOption::CheckMixContition(int itemnum, int mixitemlevel)
{
	...
	if ( itemnum <0 || itemnum > MAX_ITEM-1)
	{
		LogAddC(LOGC_RED, "MakeSetOption Check error: (itemnum:%d)", itemnum);
		return 0;
	}
...
int CSetItemOption::GenSetOption(int itemnum)
{
	if ( itemnum < 0 || itemnum > MAX_ITEM-1)
	{
		LogAddC(LOGC_RED, "GenSetType Check error: (itemnum:%d)", itemnum);
		return FALSE;
	}
