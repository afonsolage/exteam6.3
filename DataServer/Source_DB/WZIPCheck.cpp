...
BOOL CWZIPCheck::ReadIP(char* lpszFileName)
{
	...
	try
	{
		...
		while( true )
		{
			...
			while( true )
			{
				if(iType == NAME)
				{
					...
					LogAddC(LOGC_GREEN, "[ANTI-HACKING][IP LIST] IP : [%d.%d.%d.%d] Length : %d", 
		...
		LogAddC(LOGC_GREEN, "[IP List Load ] - %s file is Loaded", lpszFileName);
	}
	catch(...)
	{
...
BOOL CWZIPCheck::CheckIP(char* sIP)
{
	...
	{
		LogAddC(LOGC_RED, "Not Loading Allowable IP LIST!!");
		return TRUE;
	}
