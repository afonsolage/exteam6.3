...
void CServerManager::AddServer(int index, char* ip, SOCKET socket) // OK
{
	...
	LogAdd("[ServerManager][%d] AddServer (%s)",this->m_index,this->m_IpAddr);
}

void CServerManager::DelServer() // OK
{
	LogAdd("[ServerManager][%d] DelServer (%s)",this->m_index,this->m_IpAddr);
...
void CServerManager::SetServerInfo(char* name,WORD port,WORD code) // OK
{
	...
	LogAdd("[ServerManager][%d] ServerInfo (%s) (%d) (%d)",this->m_index,this->m_ServerName,this->m_ServerPort,this->m_ServerCode);
}
