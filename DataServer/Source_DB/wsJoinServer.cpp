...
BOOL CwsJoinServer::CreateServer(char *ip_addr, WORD port, DWORD WinServerMsg, DWORD WinClientMsg)
{
	...
	{
		LogAddC(LOGC_RED, "Server bind error %d", WSAGetLastError());
    ...
    if( nResult == SOCKET_ERROR) {
		closesocket(m_socket);
		LogAddC(LOGC_RED, "Server listen error %d", WSAGetLastError());
		return FALSE;
    }
    ...
    if( nResult == SOCKET_ERROR) {
		closesocket(m_socket);
		LogAddC(LOGC_RED, "Server WSAAsyncSelect error %d", WSAGetLastError());
        return FALSE;
    }
...
BOOL CwsJoinServer::DataSend(short uindex, char *buf, int len)
{
	...
	return gSocketManager.DataSend(uindex,(BYTE*)&buf,len);
#else
	...
	while( 1 )
	{
		...
		{ //line:30
			if( WSAGetLastError() != WSAEWOULDBLOCK )
			{
				...
				LogAddC(LOGC_RED, "error : %d (%s %d)", WSAGetLastError(), __FILE__, __LINE__);
				Close(socket);
				return FALSE;
			}
			else
			{//line:42
				...
				if( ( (*sendbuflen)+nLeft) > MAX_SENDBUFSIZE ) 
				{
					...
					LogAddC(LOGC_RED, "error : (%d) 버퍼가 꽉차서 더이상 저장할수 없다.. 사용자 접속 종료 시킨다. %d", uindex, ((*sendbuflen)+nLeft));
					Close(socket);
					return FALSE;
				}
...
int CwsJoinServer::DataRecv(SOCKET socket)
{
	...
	{
		LogAddC(LOGC_RED, "Recv result is zero (%d)", WSAGetLastError());
		return 1;
	}
	if( nResult == SOCKET_ERROR )
	{
