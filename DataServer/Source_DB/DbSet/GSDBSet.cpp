...
DWORD CGSDBSet::GetPartyCount()
{
	CString qSQL;

	qSQL.Format("SELECT PartyCount FROM GameServerInfo WHERE Number=0");
	...
	DWORD Resul = m_Query.GetInt("PartyCount");
...
void CGSDBSet::SetPartyCount(unsigned int count)
{
	CString qSQL;

	qSQL.Format("UPDATE GameServerInfo SET PartyCount = %d WHERE Number=0", count);
