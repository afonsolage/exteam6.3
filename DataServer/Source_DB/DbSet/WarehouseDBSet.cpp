...
BOOL CWarehouseDBSet::Connect()
{
	...
	{
		LogAddC(LOGC_RED, "Warehouse database connect fail.");
		return FALSE;
	}
...
BOOL CWarehouseDBSet::Save(char* AccountID, BYTE* itembuf, int money, int pw, int Number)
{
	CString qSQL;
	...
	if (m_DBQuery.Exec(qSQL) == FALSE)
	{
		LogAddC(LOGC_RED, "Money Set error [%d] %s",money,AccountID);
	}
...
BOOL CWarehouseDBSet::Load(char* AccountID, BYTE* itembuf, int& money, BYTE& _dbversion, short& pw, int Number)
{
	CString qSQL;
	...
	if (m_DBQuery.Exec(qSQL) == TRUE)
	{
		...
		{
			money = 0;
			LogAddC(LOGC_RED, "error-L2 : get money error %s", AccountID);
		}
	...
	{
		money = 0;
		LogAddC(LOGC_RED, "error-L2 : [%s] %s", AccountID, qSQL);
	}
