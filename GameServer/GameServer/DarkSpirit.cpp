...
void CDarkSpirit::Set(int aIndex, CItem * pPetItem)
{
	...
	this->m_AttackDamageMin = petitemlevel * ExConfig.Pet.RavenAttackDamageMinLvl + leadership/ExConfig.Pet.RavenAttackDamageMinCmd + 180;
	this->m_AttackDamageMax = petitemlevel * ExConfig.Pet.RavenAttackDamageMaxLvl + leadership/ExConfig.Pet.RavenAttackDamageMaxCmd + 200;
	this->m_AttackSpeed = petitemlevel * 4 / 5 + leadership / 50 + 20;
	this->m_SuccessAttackRate= petitemlevel + petitemlevel / 15 + 1000;
		...
		if ( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
		{
			if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
			{
				if ( lpObj->MapNumber == MAP_INDEX_CASTLESIEGE && lpTargetObj->MapNumber == MAP_INDEX_CASTLESIEGE )
				{
					if( lpObj->m_btCsJoinSide == lpTargetObj->m_btCsJoinSide)
					{
						//int PetAttackByNPC = GetPrivateProfileInt("CastleSiegePet","PetAttackByNPC",20,"..\\Data\\ExData\\Skills.ini");
						AttackDamage = ( AttackDamage * ExConfig.Character.PetCastleAttackByNPC ) / 100;		//AttackDamage = AttackDamage * 20 / 100;
					}
					else if(g_ShieldSystemOn == FALSE)
					{
						//int PetAttackByPlayer = GetPrivateProfileInt("CastleSiegePet","PetAttackByPlayer",40,"..\\Data\\ExData\\Skills.ini");
						AttackDamage = AttackDamage * ExConfig.Character.PetCastleAttackByPlayer / 100;	//AttackDamage = AttackDamage * 40 / 100;
					}
				}
			}
		}
