...
void GameShop::RequestPoint(int aIndex)	//-> Complete
{
	...
	{
		pPoint.WCoinC[0]	= (double)lpUser->ExCred;
		pPoint.WCoinC[1]	= (double)lpUser->ExCred;
	}
	else
	{
	...
	case 0:	//-> WCoinC
		{
			...
			if( g_ExLicense.CheckUser(SILVER1) || g_ExLicense.CheckUser(SILVER2))
			{
				if( lpUser->ExCred >= lpItem->Price )
				{
					lpUser->ExCred	-= lpItem->Price;
					pBuy.Result = false;
	...
	case 0:	//-> WCoinC
		{
			...
			if(g_ExLicense.CheckUser(SILVER1) || g_ExLicense.CheckUser(SILVER2))
			{
				if( lpUser->ExCred >= lpPackage->Price )
				{
					lpUser->ExCred	-= lpPackage->Price;
					pBuy.Result = false;
				}
			}
			else
...
void GameShop::DGGetPoint(GAMESHOP_DG_GET_POINT * aRecv)
{
	...
	//-> Send Client
	ExUserDataSend(aRecv->UserIndex);
