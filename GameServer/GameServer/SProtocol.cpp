...
void JGPAccountRequest(SDHP_IDPASSRESULT * lpMsg)
{
	...
	{
		LogAddC(2, "[SProtocol] Login result received for a non connect object: %d", aIndex);
		return;
	}
	...
	case 3:
	{
		...
		{
			CloseClient(aIndex);
			g_MUHelperOffline.CloseOfflineUser(aIndex, false);
			return;
		}
#if(_RECONNECT_)
		if (gObj[aIndex].m_ConnectEx)
			...
			for (auto it = g_PlayerMaps.begin(); it != g_PlayerMaps.end(); it++)
			{
				...
				for (auto iIt = it->second.begin(); iIt != it->second.end(); iIt++)
				{
					...
					if (g_MUHelperOffline.IsOffline(anotherIdx))
					{
						...
						//And close the client so he can try to connect again and try to login again
						CloseClient(aIndex);
						gObjDel(aIndex);
						return;
					}
					...
					{
						//This should not happen. There is some reconnect client trying to connect 
						//on a account that is already connected and isn't offline. Let's log and 
						//and ignore the reconnect for security reasons
						LogAddC(2, "[SProtocol] Invalid reconnect state! Trying to reconnect on a already connected account! %s", szId);
					}
				}
			}

			//If there is already someone connect but we didn't find on this GS, it means it is on another GS
			//So let's disconnect this reconnect try
...
void BroadCastMessage(BroadCastMessageInfo* lpResult)
{
	...
	break;
	case 2:
		MessageSendAllEx(ExConfig.Command.PostColor, lpResult->Sender, "%s %s", ExConfig.Command.PostPrist, lpResult->Text);
		break;
	}
