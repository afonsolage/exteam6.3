...
int PartyClass::Create(int usernumber, int dbnumber, int level)
{
	...
	while ( true )
	{
		if ( this->m_PartyS[n].Count == 0 )
		{
			...
			this->m_PartyS[n].m_NextLootIdx = 0;
...
int PartyClass::GetNextLootMember(int partyNumber)
{
	...
	int nextIdx = m_PartyS[partyNumber].m_NextLootIdx;

	while(m_PartyS[partyNumber].Number[nextIdx] == -1)
	{
		nextIdx++;
		nextIdx += m_PartyS[partyNumber].Count;
	}

	int result = m_PartyS[partyNumber].Number[nextIdx];
	nextIdx++;
	nextIdx += m_PartyS[partyNumber].Count;
