...
void gObjMonsterSetHitDamage(LPOBJ lpObj, int hit_player, int hit_damage)
{
	for (auto it = lpObj->sHD.begin(); it != lpObj->sHD.end(); it++)
	{
		if (it->number >= 0)
		{
			if (it->number == hit_player)
			{
				it->HitDamage += hit_damage;
				it->LastHitTime = GetTickCount();

				if (it->HitDamage > lpObj->MaxLife)
					it->HitDamage = lpObj->MaxLife;
	...
	lpObj->sHD.emplace_back(hit_player, hit_damage, GetTickCount());
}
...
int gObjMonsterTopHitDamageUser(LPOBJ lpMonObj)
{
	...
	{
		if (it->number >= 0 && gObjIsConnected(it->number) && it->HitDamage > MaxHitDamage)
		{
			MaxHitDamage = it->HitDamage;
			MaxHitDamageUser = it->number;
		}
	}
...
int	gObjMonsterLastHitDamageUser(LPOBJ lpMonObj, int & hitindex)
{
	...
	for (auto it = lpMonObj->sHD.begin(); it != lpMonObj->sHD.end(); it++)
	{
		if (it->number >= 0 && gObjIsConnected(it->number) && it->LastHitTime > LastHitTime)
		{
...
int gObjGuardSearchEnemy(LPOBJ lpObj)
{
	...
	for (n = 0; n < MAX_VIEWPORT_MONSTER; n++)
	{
		...
		if (tObjNum >= 0)
		{
			...
			if (ExConfig.PvP.ExPkPartySystem && gObj[tObjNum].PartyNumber >= 0) //Season 2.5 add-on
			{
			...
			if (OBJMAX_RANGE(aIndex))
			{
				...
				else
				{
...
#endif
					{
						...
						else if (wall == 0x02) // ƒ≥∏Ø≈Õ∞° æ’ø° ¿÷¿∏∏È..
						{
						}
						else {	// ∫Æ¿Ã∞≈≥™ ¡ﬂ∞£ø° ∏∑«Ù ¿÷¥Ÿ∏È..
							//lpObj->TargetNumber = aIndex;	// ∞¯∞›ªÛ¥Î∑Œ ¡ˆ¡§«œ∞Ì	
						}
					}
				}
	...
	if (lpObj->m_ActState.Emotion == 0)
	{
		if (lpObj->m_Attribute)
		{
			if (lpObj->m_ActState.Attack)
			{
				...
				lpObj->TargetNumber = -1;
				lpObj->NextActionTime = 500;
				//if( lpObj->m_Attribute == 100 )	LogAdd("∞¯∞›¡ﬂ ¿Ãæ˙¥Ÿ∏È ¿·±Ò ΩÆ¥Ÿ.");
			}
			...
			else
			{
				if (lpObj->m_Attribute == 100)
				{
					...
					if (lpObj->m_Attribute == 100)
					{
						//if( lpObj->TargetNumber>=0) LogAdd("¿˚¿ª √£æ∆ ¿Ãµø«—¥Ÿ.");
					}
#ifdef IMPERIAL_CONFLICT_NEW
		...
		if (lpObj->TargetNumber >= 0 && lpObj->PathStartEnd == 0)
		{
			...
			if (dis <= attackrange)
			{
				...
				if (MapC[map].CheckWall(lpObj->X, lpObj->Y, gObj[tuser].X, gObj[tuser].Y) == TRUE)
				{
					...
					if (lpObj->m_Attribute == 100) {
						//LogAdd("±Ÿ√≥∑Œ ∞°º≠ ∞¯∞›«—¥Ÿ. ¥Ÿ¿Ω æ◊º± Ω√∞£ %d",lpObj->NextActionTime);
					}
				}
...
void gObjRefillMonsterHP(LPOBJ lpMonsterObj, int iRefillHPSec)
{
	...
	if (lpMonsterObj->Class == 275)
	{
		...
		if (lpMonsterObj->iObjectSecTimer > giKundunRefillHPTime)
		{
			...
			KUNDUN_EVENT_LOG.Output("[KUNDUN] HP Log -> [%d]»∏∫π  [%d]»∏∫πΩ√∞£  [%7.0f]«ˆ¿ÁHP", iRefillHP, giKundunRefillHPTime, lpMonsterObj->Life);
			lpMonsterObj->iObjectSecTimer = 0;
		}
		else
		{
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if (g_CustomQuestSystem.IsEnabled() && g_CustomQuestSystem.MonsterKiller(lpObj->m_Index, lpTargetObj->m_Index))
	{
		//return;
	}
	else if (g_ExWinQuestSystem.MonsterKiller(lpObj->m_Index, lpTargetObj->m_Index))
	{
		return;
	}
#endif
	...
	{
		int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		int iMaxCount = ExConfig.CommonServer.CountItemDropErohim;
		//if(g_ExLicense.CheckUser(eExUB::drynea))
		//{
		//	iMaxCount = ExConfig.DryneaConfig.CountErohimItemsDrop;
		//}
		...
		for (int i = 0; i < iMaxCount; i++)
		{
			...
			//		if ( (rand()%10000) < ExConfig.DryneaConfig.ErohimItemsDrop )
			//		{
			//#endif
	...
	if (lpObj->Class == 275)
	{
		...
		int iMaxNumOfRewardItem = ExConfig.CommonServer.CountItemDropKundun;

		if (g_ExLicense.CheckUser(eExUB::drynea))
		{
			iMaxNumOfRewardItem = ExConfig.DryneaConfig.CountKundumItemsDrop;
		}
	...
	if (gObjGoldenMonsterDieGiveItem(lpObj, lpTargetObj)) return;
	...
	if (ExConfig.CommonServer.ExlDropRate > rand() % 10000)//if ( ExtDropPer == TRUE )
	{
		ExtDropPer = TRUE;
	...
	if (item_drop)
	{
		...
		{
			option1rand = gItemDropSkillRate;
			option2rand = gItemDropLuckRate;
			option3rand = rand() % 100;
			optionc = rand() % 3;
			NOption = 0;
		}
		...
		{
		case 0:
			if (option3rand < gItemDropOpt12Rate)
				Option3 = 3;
			break;
		case 1:
			if (option3rand < gItemDropOpt8Rate)
				Option3 = 2;
			break;
		case 2:
			if (option3rand < gItemDropOpt4Rate)
				Option3 = 1;
			break;
		}
		...
		if (PartyLootTime > 0)
		{
			LPOBJ lpOwner = &gObj[MaxHitUser];

			if (lpOwner != NULL && gObjIsConnectedEx(MaxHitUser) && lpOwner->PartyNumber >= 0)
			{
				int nextMemberIndex = gParty.GetNextLootMember(lpOwner->PartyNumber);

				if (nextMemberIndex != -1)
				{
					MaxHitUser = nextMemberIndex;
				}
				else
				{
					//Just a safety check to avoid infinity loop
					int max_loop = MAX_USER_IN_PARTY;

					while (gObjCalDistance(lpOwner, &gObj[nextMemberIndex]) > 10 && max_loop >= 0)
					{
						nextMemberIndex = gParty.GetNextLootMember(lpOwner->PartyNumber);
						max_loop--;
					}
				}
			}
		...
		else
		{
			if (lpTargetObj->PartyNumber != -1)
			{
				...
				if (ZenDropBonusPercPerMember > 0) {
					...
					//The bÙnus count is only for 2 or more members near
					if (nearCount > 1) {
				...
				lpTargetObj->DbClass == 35 || lpTargetObj->DbClass == 50 || lpTargetObj->DbClass == 66 || lpTargetObj->DbClass == 83 || lpTargetObj->DbClass == 98))
			{
				...
				{
					if (ExConfig.PrideMu.MasterMoney < 1)
					{
						money = money * ExConfig.CommonServer.Ex_MasterDropZen;
					}
					else if (ExConfig.PrideMu.DiapasonMasterMoney < 1)
					{
						money = ExConfig.PrideMu.MasterMoney;
					}
					else
					{

						money = (rand() % ExConfig.PrideMu.DiapasonMasterMoney) + ExConfig.PrideMu.MasterMoney;
					}
			...
			if (g_ExLicense.user.WinQuest) gWinQuestSystem.ZenOption(lpObj->m_Index, money);

			gExclusiveOptions.DropZen(lpObj->m_Index, money);

#if(ENABLE_PETEX)
			g_PetEx.Zen(lpObj->m_Index, money);
#endif
			...
			money = (money * ExConfig.CommonServer.ProcentDropZen) / 100;

			if (ExConfig.CommonServer.MaxDropZen > 0)
			{
			...
			g_PremiumSystemEx.UserZenPlus(lpObj->m_Index, money);
#endif
...
{
	std::vector<std::tuple<int, int, int, int>> drops; //type, level, X, Y
	std::string name;

	switch (lpObj->Class)
	{
	case eMOB_GOLDEN_BUDGE_DRAGON:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType43, ExConfig.GoldenDrop.ItemIndex43),
			ExConfig.GoldenDrop.level43,
			lpObj->X,
			lpObj->Y);
		name = "Golden Budge Dragon";
		break;
	case eMOB_GOLDEN_GOBLIN:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType78, ExConfig.GoldenDrop.ItemIndex78),
			ExConfig.GoldenDrop.level78,
			lpObj->X,
			lpObj->Y);
		name = "Golden Goblin";
		break;
	case eMOB_GOLDEN_DARK_KNIGHT:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType493, ExConfig.GoldenDrop.ItemIndex493),
			ExConfig.GoldenDrop.level493,
			lpObj->X,
			lpObj->Y);
		name = "Golden Dark Night";
		break;
	case eMOB_GOLDEN_TITAN:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType53, ExConfig.GoldenDrop.ItemIndex53),
			ExConfig.GoldenDrop.level53,
			lpObj->X,
			lpObj->Y);
		name = "Golden Titan";
		break;
	case eMOB_GOLDEN_SOLDIER:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType54, ExConfig.GoldenDrop.ItemIndex54),
			ExConfig.GoldenDrop.level54,
			lpObj->X,
			lpObj->Y);
		name = "Golden Soldier";
		break;
	case eMOB_GOLDEN_LIZARD_KING:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType80, ExConfig.GoldenDrop.ItemIndex80),
			ExConfig.GoldenDrop.level80,
			lpObj->X,
			lpObj->Y);
		name = "Golden Lizard King";
		break;
	case eMOB_GOLDEN_VEPAR:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType81, ExConfig.GoldenDrop.ItemIndex81),
			ExConfig.GoldenDrop.level81,
			lpObj->X,
			lpObj->Y);
		name = "Golden Vepar";
		break;
	case eMOB_GOLDEN_DEVIL:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType494, ExConfig.GoldenDrop.ItemIndex494),
			ExConfig.GoldenDrop.level494,
			lpObj->X,
			lpObj->Y);
		name = "Golden Devil";
		break;
	case eMOB_GOLDEN_TANTALOS:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType82, ExConfig.GoldenDrop.ItemIndex82),
			ExConfig.GoldenDrop.level82,
			lpObj->X,
			lpObj->Y);
		name = "Golden Tantalos";
		break;
	case eMOB_GOLDEN_WHEEL:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType83, ExConfig.GoldenDrop.ItemIndex83),
			ExConfig.GoldenDrop.level83,
			lpObj->X,
			lpObj->Y);
		name = "Golden Wheel";
		break;
	case eMOB_GOLDEN_GOLEM:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType495, ExConfig.GoldenDrop.ItemIndex495),
			ExConfig.GoldenDrop.level495,
			lpObj->X,
			lpObj->Y);
		name = "Golden Golem";
		break;
	case eMOB_GOLDEN_CRUST:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType496, ExConfig.GoldenDrop.ItemIndex496),
			ExConfig.GoldenDrop.level496,
			lpObj->X,
			lpObj->Y);
		name = "Golden Crust";
		break;
	case eMOB_GOLDEN_SATYRUS:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType497, ExConfig.GoldenDrop.ItemIndex497),
			ExConfig.GoldenDrop.level497,
			lpObj->X,
			lpObj->Y);
		name = "Golden Satyrus";
		break;
	case eMOB_GOLDEN_TWIN_TALE:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType498, ExConfig.GoldenDrop.ItemIndex498),
			ExConfig.GoldenDrop.level498,
			lpObj->X,
			lpObj->Y);
		name = "Golden Twin Tale";
		break;
	case eMOB_GOLDEN_IRON_KNIGHT:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType499, ExConfig.GoldenDrop.ItemIndex499),
			ExConfig.GoldenDrop.level499,
			lpObj->X,
			lpObj->Y);
		name = "Golden Iron Knight";
		break;
	case eMOB_GOLDEN_NAPIN:
		drops.emplace_back(
			ItemGetNumberMake(ExConfig.GoldenDrop.ItemType500, ExConfig.GoldenDrop.ItemIndex500),
			ExConfig.GoldenDrop.level500,
			lpObj->X,
			lpObj->Y);
		name = "Golden Napin";
		break;
	case eMOB_GOLDEN_DRAGON:
		name = "Golden Dragon!";
		for (int i = 0; i < ExConfig.GoldenDrop.countitem79; i++)
		{
			BYTE dropX = lpObj->X;
			BYTE dropY = lpObj->Y;

			if (!gObjGetRandomItemDropLocation(lpObj->MapNumber, dropX, dropY, 2, 2, 10))
			{
				dropX = lpObj->X;
				dropY = lpObj->Y;
			}
			...
			if (rnd < ExConfig.GoldenDrop.rate1_79)
			{
				level = ExConfig.GoldenDrop.level1_79;
			}
			else
			{
				level = ExConfig.GoldenDrop.level2_79;
			}

			drops.emplace_back(
				ItemGetNumberMake(ExConfig.GoldenDrop.ItemType79, ExConfig.GoldenDrop.ItemIndex79),
				level,
				dropX,
				dropY);
		}
		break;
	case eMOB_GOLDEN_GREAT_DRAGON:
		name = "Great Golden Dragon!!!";
		for (int i = 0; i < ExConfig.GoldenDrop.countitem501; i++)
		{
			BYTE dropX = lpObj->X;
			BYTE dropY = lpObj->Y;
			...
			if (rnd < ExConfig.GoldenDrop.rate1_501)
			{
				level = ExConfig.GoldenDrop.level1_501;
			}
			else
			{
				level = ExConfig.GoldenDrop.level2_501;
			}

			drops.emplace_back(
				ItemGetNumberMake(ExConfig.GoldenDrop.ItemType501, ExConfig.GoldenDrop.ItemIndex501),
				level,
				dropX,
				dropY);
		}
	...
	auto x = lpObj->X;
	auto y = lpObj->Y;
	auto maxHitUser = gObjMonsterTopHitDamageUser(lpObj);

	for (auto it = drops.begin(); it != drops.end(); it++)
	{
		auto type = std::get<0>(*it);
		auto level = std::get<1>(*it);
		auto x = std::get<2>(*it);
		auto y = std::get<3>(*it);

		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, x, y, type, (BYTE)level, (BYTE)dur,
			0, 0, 0, maxHitUser, 0, 0);
	}

	std::string killer = "Someone";

	if (maxHitUser != -1 && gObjIsConnected(maxHitUser))
	{
		LPOBJ lpKiller = &gObj[maxHitUser];
		killer = lpKiller->Name;
	}

	char tmp[256] = { 0 };
	sprintf(tmp, "%s Killed %s", killer.c_str(), name.c_str());
	AllSendServerMsg(tmp);
