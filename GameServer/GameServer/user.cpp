...
void gObjItemTextSave(LPOBJ lpObj)
{
	...
	LogAddTD("[%d][%s][%s] GameShop: WCoinC=%d WCoinP=%d GoblinPoint=%d [Info]", 
		lpObj->m_Index, lpObj->AccountID, lpObj->Name, (int)lpObj->GameShop.WCoinC, (int)lpObj->GameShop.WCoinP, (int)lpObj->GameShop.GoblinPoint);
...
#if(GS_CASTLE==1 || CS_SERVER) //Check Position in gs 77 after siege patch
	if(g_Crywolf.GetCrywolfState() == 4)
	{
...
int gObjGetLifeItemPos(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int count = 0;
	...
	for ( int n=0;n<MAIN_INVENTORY_SIZE;n++)
	{
		if ( lpObj->pInventory[n].IsItem() == TRUE )
		{
			if ( lpObj->pInventory[n].m_Type == ITEMGET(14,0) || lpObj->pInventory[n].m_Type == ITEMGET(14,2) || lpObj->pInventory[n].m_Type == ITEMGET(14,3) || lpObj->pInventory[n].m_Type == ITEMGET(14,4) )
			{
				return n;
			}
		}
