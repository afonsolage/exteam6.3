			...
			case 0xFB:
				{
					...
					switch ( lpDef->subcode )
					{
						...
						case 15:
							{
								if(g_ExLicense.user.WinQuest) gWinQuestSystem.RecvPacket(aIndex, (CGSendQuestResult*)aRecv);
							}
							break;
						case 16:
...
//#if(CUSTOM_PERSONAL_SHOP==TRUE)
								if(g_PersonalShopEx.Enable)
								{
									g_PersonalShopEx.Search(aIndex, (CG_PersonalPage*) aRecv);
								}
//#endif
							}
							break;
						...
						case 0x1A:
							g_ExUser.CGAddPoints(aIndex, (PMSG_ADD_POINTS*) aRecv);
							break;
#endif
#if(CUSTOM_ADD_POINTS==TRUE)
						case 0x26:
							g_ExUser.CGResetPoints(aIndex, (PMSG_ADD_POINTSRESET*) aRecv);
							break;
#endif
						case 0x41:
						...
						case 0x17:
							g_ExTeleport.CGResult((PMSG_CG_TELEPORTWINRES*) aRecv, aIndex);
							break;
#endif
						case 0x95:
						...
						case 0xA3:
							g_ExTeleportEvo.CGResult((PMSG_CG_TELEPORTWINRESEVO*) aRecv, aIndex);
							break;
						case 0xA4:
							g_ExTeleportEvo.CGDialog((PMSG_CG_DIALOGEVO*) aRecv, aIndex);
							break;
#endif
						case 0xAF:
							{
...
#endif
						case 0xBA:
							g_ExUser.CG_RecvAutoHPClicker((PMSG_PROTOCOL_RESULT*)aRecv, aIndex);
							break;
#if(CUSTOM_EX_QUEST_SYSTEM==TRUE)
						case 0x12:
							{
								if (g_CustomQuestSystem.IsEnabled())
									g_CustomQuestSystem.CG_AcceptQuest(aIndex, (CG_CQAcceptDone*) aRecv);
								else if (g_ExWinQuestSystem.Enable)
									g_ExWinQuestSystem.CG_AcceptQuest(aIndex, (CG_Accept_Done*) aRecv);
							}
							break;
#endif
...
#if(ADD_CTRL_KEY)
						case 0xD3:
							g_ExUser.CGCtrlKey(aIndex, (PMSG_CONTROLLER_CTRL*) aRecv);
							break;
#endif
						case 0xD7:
							g_ExUser.RecvConfig((CG_MINIMAP_PARTY*) aRecv, aIndex);
							break;
						case 0xD8:
...
void CGCheckMainRecv(PMSG_CHECK_MAINEXE_RESULT * lpMsg, int aIndex) 
{
	...
	gObj[aIndex].CheckSumTime = 0;
		...
		return;
#endif
	//int EnableCheckSum = GetPrivateProfileInt("Common","CheckSum",0,"..\\Data\\ExData\\CommonServer.ini");
...
void AllSendServerMsg( char* chatmsg) 
{
	...
	if(g_ExLicense.CheckUser(eExUB::masonX) || g_ExLicense.CheckUser(eExUB::Local))
	{
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex) 
{
	...
	{
		if( ExConfig.Command.CommandPost[i] != lpChat->chatmsg[i])
		PostConfigConflictFlag = false;
		PostConfigConflictCount++;
	};
	for(int i = 0; ExConfig.Command.AdminPostName[i] != '\0';i++)
	{
		if( ExConfig.Command.AdminPostName[i] != lpChat->chatmsg[i])
		AdminConfigConflictFlag = false;
		AdminConfigConflictCount = i;
	};
	//LogAddTD("%d", ExConfig.Command.PostColor);
			...
			if ( slen > 2 )
			{
				if ( CheckAuthorityCondition(98, lpObj) == true) //season4 changed
				{
					...
					if(g_ExLicense.CheckUser(eExUB::ulasevich) || g_ExLicense.CheckUser(eExUB::Local3) || g_ExLicense.CheckUser(eExUB::ulasevich2) ||
						g_ExLicense.CheckUser(eExUB::Almdar) || g_ExLicense.CheckUser(eExUB::Almdar2) ||
...
#if(CUSTOM_CHAT_LOG==TRUE)
					if ( gWriteChatLog && g_ExLicense.user.ChatLog)
					{
			...
			{
				{
					...
					{
						if(ExConfig.DryneaConfig.PostFloodProtect == 1)
						{
							if(lpObj->PostTickCount != 0)
							{
								if ( 1000 * ExConfig.DryneaConfig.PostFloodProtectTime >= GetTickCount() - lpObj->PostTickCount )
								{
									MsgNormal(aIndex,"Please wait %d seconds before next post", ExConfig.DryneaConfig.PostFloodProtectTime - ((GetTickCount() - lpObj->PostTickCount) / 1000));
									return;
								}
							}
						}
...
#endif
					if(ExConfig.Command.PostDelay > 0)
					{
						if ( 1000 * ExConfig.Command.PostDelay >= GetTickCount() - lpObj->PostTickCount )
						{
							MsgNormal(aIndex,"Please wait %d seconds before next post", ExConfig.Command.PostDelay - ((GetTickCount() - lpObj->PostTickCount) / 1000));
							return;
						}
					...
					//sprintf(ChatMessage, "%s %s",ExConfig.Command.PostPrist,&lpChat->chatmsg[1]);break;
					...
					if(ExConfig.Command.PostServer)
					{
						BroadCastMessageInfo lpRequest = { 0 };
						lpRequest.h.set((LPBYTE)&lpRequest, 0xCD, sizeof(lpRequest));
						lpRequest.Type = 2;
						strcat(lpRequest.Text, &lpChat->chatmsg[PostConfigConflictCount]);
						memcpy(lpRequest.Sender, lpChat->chatid, 10);		
						wsJServerCli.DataSend((PCHAR)&lpRequest, sizeof(BroadCastMessageInfo));
					}
					else
					{
						MessageSendAllEx(ExConfig.Command.PostColor,lpChat->chatid, "%s %s", ExConfig.Command.PostPrist,&lpChat->chatmsg[PostConfigConflictCount]);
					}
#else
					MessageSendAllEx(ExConfig.Command.PostColor,lpChat->chatid, "%s %s", ExConfig.Command.PostPrist,&lpChat->chatmsg[PostConfigConflictCount]);
#endif
		...
		case '!':	// Global Announcement
			if ( slen > 2 )
			{
				...
				else
				{
					...
					lpObj->Money -= ExConfig.Command.PostMoney;
					GCMoneySend(aIndex, lpObj->Money);
					MessageSendAllEx(ExConfig.Command.PostColor,lpChat->chatid, "%s %s", ExConfig.Command.PostPrist,&lpChat->chatmsg[1]);
				}
#endif
				return;
			}
...
void CSPJoinIdPassRequest(PMSG_IDPASS* lpMsg, int aIndex) 
{
	...
	if(g_ExLicense.CheckUser(eExUB::Local) || g_ExLicense.CheckUser(ulasevich) || g_ExLicense.CheckUser(ulasevich2) || 
		g_ExLicense.CheckUser(eExUB::Gredy) || g_ExLicense.CheckUser(eExUB::GredyLocal) || g_ExLicense.CheckUser(eExUB::Gredy2)
		 || g_ExLicense.CheckUser(eExUB::MU2Play) || g_ExLicense.CheckUser(eExUB::Sentinel) || g_ExLicense.CheckUser(eExUB::Escalate) || g_ExLicense.CheckUser(eExUB::eternalmu) ||
...
void CGPCharacterCreate( PMSG_CHARCREATE * lpMsg, int aIndex) 
{
	...
	if(g_ExLicense.CheckUser(eExUB::snaker) || g_ExLicense.CheckUser(eExUB::Local) || g_ExLicense.CheckUser(eExUB::masonX))
	{

	}
	else
	{
			if(!exCheckSimbol(lpMsg->Name, strlen(lpMsg->Name)))
			{
	...
	if ( lpItem->IsItem() == TRUE && lpItem->Give == false && lpItem->live == true)
	{
		...
		if ( lpItem->m_Type == ITEMGET(14,15) )
		{
			if ( MapC[map_num].ItemGive(aIndex, item_num, false) == TRUE )
			{
				...
				if (ShareZenParty && gObj[aIndex].PartyNumber >= 0)
				{
					LPOBJ lpObj = &gObj[aIndex];
...
#if(CUSTOM_EX_QUEST_SYSTEM==TRUE)
			if( g_ExWinQuestSystem.PickUpItem(aIndex,lpItem->m_Type) == 1 && MapC[map_num].ItemGive(aIndex, item_num, false) == TRUE ) 
			{
 				lpItem->Clear();
				pResult.result = -1;
				pResult.h.size -= sizeof(pResult.Data);
...
void GCUserWarehouseSend(LPOBJ lpObj) 
{
	...
	DataSend(lpObj->m_Index, SendByte, lOfs);	//Îòñûëàåò ïàêåò âåùåé â êëèåíò
...
#if(CUSTOM_PERSONAL_SHOP ==TRUE)
	if(!g_PersonalShopEx.CheckPrice(iItemPrice, iBonusPrice, aBlessCnt, aSoulCnt, aChaosCnt, aLifeCnt, iCreditPrice))
	{
...
//#if(CUSTOM_PERSONAL_SHOP==TRUE)
	if(g_PersonalShopEx.Enable)
	{
		if(!g_PersonalShopEx.PShop_CheckInventoryPrice(aIndex))
		{
...
//#endif
	...
	{
#if(_RECONNECT_==TRUE)
		g_ConnectEx.SendClose(aIndex, TRUE);
#else
		CloseClient(aIndex);
...
void CGPShopAnsBuyList(int aSourceIndex, int aTargetIndex, BYTE btResult, bool bResend) 
{
	...
	{
#if(CUSTOM_PERSONAL_SHOP==TRUE)
		g_PersonalShopEx.UpdatePSInventoryTarget(aSourceIndex, aTargetIndex);
#endif
...
//#if(CUSTOM_PERSONAL_SHOP==TRUE)
				if(g_PersonalShopEx.Enable)
				{
					pMsgIL.PShopItemValue = g_PersonalShopEx.GetMoney(aTargetIndex,n); //lpObjTarget->Inventory1[n].m_iPShopValue;
				}
				else
//#else
				{
...
//#endif
	...
	if ( gObj[number].GuildNumber > 0 )
	{
		...
		if( g_ExLicense.CheckUser(eExUB::Gredy) || 
			...
			g_ExLicense.CheckUser(eExUB::MUDev))	
		{
	...
	if(g_ExLicense.CheckUser(eExUB::Gredy) || g_ExLicense.CheckUser(eExUB::Gredy2) || g_ExLicense.CheckUser(eExUB::GredyLocal))	
	{
		...
		{
			MaxGuildMember = ExConfig.m_iGuildMaxUser;
		}
...
void GCReqmoveDevilSquare(PMSG_REQ_MOVEDEVILSQUARE* lpMsg, int aIndex) 
{
	...
	if(!ExConfig.Event.DSAllowPKPlayer)
	{
		if(lpObj->m_PK_Level >= 4)
		{
			bPlayerKiller = TRUE;
...
void CGRequestEnterBloodCastle(PMSG_REQ_MOVEBLOODCASTLE* lpMsg, int iIndex) 
{
	...
	if(ExConfig.PvP.ExPkPartySystem && gObj[iIndex].PartyNumber >= 0) //Season 2.5 add-on
	{
	...
	if(!ExConfig.Event.BCAllowPKPlayer)
	{
	...
	if ( aRecv->btRequestType == 1 )
	{
		if ( aRecv->btRelationShipType == 1 )
		{
			...
			if ( lpTargetGuildInfo->iGuildUnion != 0 && lpTargetGuildInfo->iGuildUnion == lpTargetGuildInfo->Number )
			{
				if(g_ExLicense.CheckUser(Gredy) || g_ExLicense.CheckUser(Gredy2) || g_ExLicense.CheckUser(GredyLocal))
				{
					if ( UnionManager.GetGuildRelationShipCount(lpTargetGuildInfo->iGuildUnion, 1) >= ExConfig.m_iAllianceMaxGuild  )
					{
