...
bool sortCustomBoxDrop(const CustomBoxGroup &a, const CustomBoxGroup &b)
{
	return a.rate < b.rate;
}


CCustomBoxDrop::CCustomBoxDrop(void)
{
}
...
void CCustomBoxDrop::LoadItem(LPSTR script_file)
{
	this->Init();
	...
	while (true)
	{
		...
		if (Token == 1)
		{
			...
			if (section_type == 2)
			{
				...
				Token = GetToken();
				group.exc_rate = (int)TokenNumber;
	...
	{
		std::sort(this->m_groups.begin(), this->m_groups.end(), sortCustomBoxDrop);
...
CustomBoxGroup* CCustomBoxDrop::GetGroup(int rnd)
{
...
int CCustomBoxDrop::RndRange(int min, int max)
{
	 return (rand() % ((max - min) + 1)) + min;
}

int CCustomBoxDrop::IsProcRnd(int proc, int max)
{
	return rand() % max < proc;
}

BOOL CCustomBoxDrop::DropItem(LPOBJ lpObj)
{
	float dur;
	int type;
	int level;
	...
	CustomBoxGroup* lpGroup = GetGroup(rnd);

	if (lpGroup == NULL || lpGroup->rate == 0 || lpGroup->items.size() == 0)
	{
		LogAddTD("[%s][%s][CustomBoxDrop][%s] : (%d)(X:%d/Y:%d) Invalid group at rnd %d",
	...
	if (optRnd < lpGroup->opt16_rate)
		Option3 = 4;
	else if (optRnd < lpGroup->opt12_rate)
		Option3 = 3;
	else if (optRnd < lpGroup->opt8_rate)
		Option3 = 2;
	else if (optRnd < lpGroup->opt4_rate)
		Option3 = 1;
	else
		Option3 = 0;
	...
	LogAddTD("[%s][%s][CustomBoxDrop][%s] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d",
