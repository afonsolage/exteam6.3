...
{
	MessaageAllGlobal(g_ExText.GetText(254));
			...
			if(g_ExLicense.CheckUser(eExUB::Gredy) || g_ExLicense.CheckUser(eExUB::GredyLocal) || g_ExLicense.CheckUser(eExUB::Gredy2))
			{
				...
				{			
						if(Time[i].Hour == t.wHour && ((Time[i].Min-1) == t.wMinute))
							MessaageAllGlobal(ExConfig.MessageConfig.DropEvent, 1);
				}
				...
				if(Time[i].Min > 5)
				{
					for(int j=1; j <= 5; j++)
					{
						...
						if(Time[i].Hour == t.wHour && (Time[i].Min-j) == t.wMinute)
							MessaageAllGlobal(ExConfig.MessageConfig.DropEvent, j);
					}
				}
				else if(Time[i].Min == 4)
				{
						if(Time[i].Hour-1 == t.wHour && 59 == t.wMinute)
							MessaageAllGlobal(ExConfig.MessageConfig.DropEvent, 5);
						if(Time[i].Hour == t.wHour && 0 == t.wMinute)
							MessaageAllGlobal(ExConfig.MessageConfig.DropEvent, 4);
						if(Time[i].Hour == t.wHour && 1 == t.wMinute)
							MessaageAllGlobal(ExConfig.MessageConfig.DropEvent, 3);
						if(Time[i].Hour == t.wHour && 2 == t.wMinute)
							MessaageAllGlobal(ExConfig.MessageConfig.DropEvent, 2);
