...
BOOL NpcTalk(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if(g_ExLicense.user.WinQuest) gWinQuestSystem.NPCClicker(lpNpc,lpObj);
	...
	if( g_PersonalShopEx.NPC_Dialog(lpObj->m_Index,lpNpc->m_Index) )
	{
		return true;
	}
#endif
	...
	if(g_ExTeleport.Dialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif
#if(CUSTOM_EX_TELEPORT_EVO)
	if(g_ExTeleportEvo.Dialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif
	...
	if (g_CustomQuestSystem.IsEnabled() && g_CustomQuestSystem.Dialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
	else if(g_ExWinQuestSystem.Dialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif
