...
void CItem::Convert(int type, BYTE Option1, BYTE Option2, BYTE Option3,  BYTE Attribute2, BYTE SetOption, BYTE ItemEffectEx, BYTE SocketOption[5], BYTE SocketIndex, BYTE PeriodItemOption, BYTE DbVersion)
{
	...
	if ( gSetItemOption.IsSetItem(_type) != 0 && SetOption != 255 && SOption != 0)
	{
		...
		{
			//int ExExcellentAncient = GetPrivateProfileInt("Items","ExcellentAncient",1,"..\\Data\\ExData\\CommonServer.ini");
			if(!ExExcellentAncient)
				this->m_NewOption = 0;
			Attribute2 = 0;
	...
	if(g_SocketOption.IsActive(this) == 1)
	{
		...
		{	//Zero SET
			//this->m_ItemOptionEx = 0;
			//int ExAncientSocket = GetPrivateProfileInt("Items","AncientSocket",1,"..\\Data\\ExData\\CommonServer.ini");
			if(!ExAncientSocket)
				this->m_SetOption = 0;
			
			//int ExExcellentSocket = GetPrivateProfileInt("Items","ExcellentSocket",1,"..\\Data\\ExData\\CommonServer.ini");
			if(!ExExcellentSocket)
			{
				Attribute2 = 0;
				this->m_NewOption = 0;
			}
