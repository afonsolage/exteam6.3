...
LPFRIEND_MASTER CFriendSystem::GetFriendMaster(char *szMaster)
{
	...
	std::map<std::string, FRIEND_MASTER, strCmp>::iterator it;
...
{
	if ( this->m_MailDB.Connect(3, g_ExDSInfo.dbDSN, SQLUser, SQLPass) == FALSE )
	{
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[ERROR] - FRIEND DB CANNOT CONNECT TO MSSQL");
		return FALSE;
	}

	if ( this->m_FriendDB.Connect(3, g_ExDSInfo.dbDSN, SQLUser, SQLPass) == FALSE )
	{
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[ERROR] - FRIEND DB CANNOT CONNECT TO MSSQL");
		return FALSE;
	}

	g_Window.ServerLogAdd(ST_EXDATASERVER, "[SUCCESS] - FRIEND DB CONNECT MSSQL SUCCESS");
...
int CFriendSystem::ExDBGetIndexByCode(int iServerCode)
{
	...
	for (int i = 0; i < MAX_SERVEROBJECT; i++)
	{
		if (gSObj[i].Connected == 2 
			...
			&& gSObj[i].ServerCode == iServerCode)
		{
			return i;
		}
	}
...
BOOL CFriendSystem::GetDBGuidAndMemoTotal(char *szName, int& guid, int& memo_total)
{
	...
	if(this->m_FriendDB.ExecQuery("SELECT GUID, MemoTotal FROM T_FriendMain where Name = '%s'", szName))
	{
		...
		{
			guid = this->m_FriendDB.GetInt("GUID");
			memo_total = this->m_FriendDB.GetInt("MemoTotal");
			res = TRUE;
		}
	}
...
BOOL CFriendSystem::GetDBFriendList(char *szMaster)
{
	...
	if(this->m_FriendDB.ExecQuery("SELECT FriendGuid, FriendName, Del FROM T_FriendList where GUID = %d", lpMaster->m_iGUID))
	{
		while(this->m_FriendDB.Fetch() != SQL_NO_DATA)
		{
			...
			{
				g_Window.ServerLogAdd(ST_EXDATASERVER, "DB contains more then %d friends for [%s] character", this->m_iMaxFriends, szMaster);
				break;
			}

			this->m_FriendDB.GetStr("FriendName", szFriend);
...
void CFriendSystem::FriendListSend(int aIndex, char *szMaster)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend List Send] Name[%s].", szMaster);
	lpMaster = GetFriendMaster(szMaster);
	if(lpMaster == NULL)
	{
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend List Send] FrienMaster[%s] not found.", szMaster);
		return;
	}
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend List Send] Friend Count[%d].", cnt);
	for(int i=0; i < cnt; i++)
	{
		...
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend List Send] Friend[%s] send.", szFriend);
	}
...
BOOL CFriendSystem::GetDBWaitFriend(char *szMaster, OUT char *szWaitFriend)
{
	...
	if(this->m_FriendDB.ExecQuery("SELECT FriendName FROM T_WaitFriend where GUID=%d", lpMaster->m_iGUID))
	{
		if(this->m_FriendDB.Fetch() != SQL_NO_DATA)
		{
			this->m_FriendDB.GetStr("FriendName", szWaitFriend);
			res = TRUE;
		}
	}
...
void CFriendSystem::FriendListRequest(int aIndex, FHP_FRIENDLIST_REQ* lpMsg)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend List Request] Name[%s].", szMaster);
...
BOOL CFriendSystem::AddFriend(char *szMaster, char *szFriend)
{
	...
	{
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[%s]'s friend list is full - %d", szMaster, this->m_iMaxFriends);
		return FALSE;
	}

	lpMaster->m_vecFriends.push_back(szFriend);

	g_Window.ServerLogAdd(ST_EXDATASERVER, "[%s] became [%s]'s friend.", szFriend, szMaster);
	return TRUE;
}
...
int CFriendSystem::AddDBWaitFriend(char *szMaster, char *szFriend)
{
	...
	if(this->m_FriendDB.ExecQuery("EXEC WZ_WaitFriendAdd '%s', '%s'", szMaster, szFriend))
	{
		if(this->m_FriendDB.Fetch() != SQL_NO_DATA)
		{
			...
			{
				if(res == 6)
					g_Window.ServerLogAdd(ST_EXDATASERVER, "[%s]'s level is too low.", szFriend);
				else if(res == 3)
					g_Window.ServerLogAdd(ST_EXDATASERVER, "GUID for [%s] doesn't exists.", szFriend);
				else if(res != 8)
					g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_WaitFriendAdd returned: %d", res);
			}
		}
		else
			g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_WaitFriendAdd no result returned.");
	}
	else
		g_Window.ServerLogAdd(ST_EXDATASERVER, "Error executing WZ_WaitFriendAdd.");
...
void CFriendSystem::FriendAddRequest( int aIndex, FHP_FRIEND_ADD_REQ* lpMsg)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend Add Request] Name[%s] FriendName[%s].", szMaster, szFriend);
	...
	if(strcmp(szMaster, szFriend) == 0)
	{
		...
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend Add Request] Send Result [%d].", Result.Result);
...
int CFriendSystem::DelDBWaitFriend(char *szMaster, char *szFriend)
{
	...
	if(this->m_FriendDB.ExecQuery("EXEC WZ_WaitFriendDel '%s', '%s'", szMaster, szFriend))
	{
		if(this->m_FriendDB.Fetch() != SQL_NO_DATA)
		{
			res = this->m_FriendDB.GetResult(0);
			if(res != 0)
				g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_WaitFriendDel returned: [%d]", res);
		}
		else
			g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_WaitFriendDel no result returned.");
	}
	else
		g_Window.ServerLogAdd(ST_EXDATASERVER, "Error executing WZ_WaitFriendDel.");
...
BOOL CFriendSystem::AddDBFriend(char *szMaster, char *szFriend)
{
	...
	if(this->m_FriendDB.ExecQuery("EXEC WZ_FriendAdd '%s', '%s'", szMaster, szFriend))
	{
		if(this->m_FriendDB.Fetch() != SQL_NO_DATA)
		{
			res = this->m_FriendDB.GetResult(0);
			if(res != 1)
				g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_FriendAdd returned: %d", res);

		}
		else
			g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_FriendAdd no result returned.");

	}
	else
		g_Window.ServerLogAdd(ST_EXDATASERVER, "Error executing WZ_FriendAdd.");
...
BOOL CFriendSystem::DelDBFriend(char *szMaster, char *szFriend)
{
	...
	if(this->m_FriendDB.ExecQuery("EXEC WZ_FriendDel '%s', '%s'", szMaster, szFriend))
	{
		if(this->m_FriendDB.Fetch() != SQL_NO_DATA)
		{
			res = this->m_FriendDB.GetResult(0);
			if(res != 1)
				g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_FriendDel returned: %d", res);

		}
		else
			g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_FriendDel no result returned.");

	}
	else
		g_Window.ServerLogAdd(ST_EXDATASERVER, "Error executing WZ_FriendDel.");
...
int CFriendSystem::WaitFriendAdd(char *szMaster, char *szFriend)
{
	...
	if(lpMaster->m_vecFriends.size() == this->m_iMaxFriends)
	{
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[%s]'s friend list is full - %d", szMaster, m_iMaxFriends);
		return 3;
	}
...
void CFriendSystem::WaitFriendAddRequest( int aIndex,  FHP_WAITFRIEND_ADD_REQ* lpMsg)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[WaitFriend Add Request] Name[%s], FriendName[%s].", szMaster, szFriend);

	if(lpMsg->Result == 0)
	{
		Result.Result = 2;
		DelDBWaitFriend(szMaster, szFriend);
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[WaitFriend Add Request] [%s] declined [%s]'s request to be a friend.", szMaster, szFriend);
		DataSend(aIndex, (LPBYTE)&Result, Result.h.size);
		return;
	}

	g_Window.ServerLogAdd(ST_EXDATASERVER, "[WaitFriend Add Request] [%s] accepted [%s]'s request to be a friend.", szMaster, szFriend);
...
void CFriendSystem::FriendStateClientRecv( int aIndex, FHP_FRIEND_STATE_C* lpMsg)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend State Receive] Name: [%s], State: [%d].", szMaster, lpMsg->State);
	LPFRIEND_MASTER lpMaster; 
	lpMaster = GetFriendMaster(szMaster);
	if(lpMaster == NULL)
	{
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend State Receive] error: FriendMaster [%s] not found.", szMaster);
		return;
	}
...
BOOL CFriendSystem::DelFriend(char *szMaster, char *szFriend)
{
	...
	for(int i=0; i < cnt; i++)
	{
		if(strcmp(lpMaster->m_vecFriends[i].m_szName, szFriend) == 0)
		{
			...
			g_Window.ServerLogAdd(ST_EXDATASERVER, "[%s] dropped [%s] from his friend list.", szMaster, szFriend);
			break;
		}
...
void CFriendSystem::FriendDelRequest( int aIndex, FHP_FRIEND_ADD_REQ* lpMsg)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend Delete Request] Name[%s], FriendName[%s].", szMaster, szFriend);
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Friend Delete Request] Send Result [%d].", Result.Result);
	DataSend(aIndex, (LPBYTE)&Result, Result.h.size);
...
void CFriendSystem::MemoListSend(int aIndex, WORD wNumber, char *szName)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail List Send] Name: [%s].", szMaster);
	int guid = GetFriendGuid(szMaster);
	if(guid == -1)
	{
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail List Send] error: GUID = -1.");
		return;
	}
	...
	if(this->m_MailDB.ExecQuery("SELECT MemoIndex, FriendName, wDate, Subject, bRead FROM T_FriendMail where GUID=%d", guid))
	{
		while(this->m_MailDB.Fetch() != SQL_NO_DATA)
		{
			...
			auto friendName = this->m_MailDB.GetString("FriendName");
			auto date = this->m_MailDB.GetString("wDate");
			auto subject = this->m_MailDB.GetString("Subject");

			head.MemoIndex = this->m_MailDB.GetInt("MemoIndex");
			memcpy(head.SendName, friendName.c_str(), friendName.size());
			memcpy(head.Date, date.c_str(), date.size());
			memcpy(head.Subject, subject.c_str(), subject.size());
			head.read = this->m_MailDB.GetInt("bRead");
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail List Send] Mail Count: %d.", cnt);
}
...
void CFriendSystem::FriendMemoListReq( int aIndex, FHP_FRIEND_MEMO_LIST_REQ* lpMsg)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail List Request] Name[%s].", szName);
		...
		szSendName, szRecvName, NULL, lpMemoSendHdr->Dir,  lpMemoSendHdr->Action))
	{
		if(this->m_MailDB.Fetch() != SQL_NO_DATA)
		{
			...
			if(memo_index <= 10)
			{
				...
				g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_WriteMail returned: %d", memo_index);
				return - 1;
			}

			if(guid < 0)
			{
				g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_WriteMail returned guid: %d", guid);
				return - 1;
			}
			...
			this->m_MailDB.WriteBlob(szMsg, Photo, btPhotoSize);

			ZeroMemory(szMsg, 1500);
			wsprintf(szMsg, "UPDATE T_FriendMail SET Subject=? where MemoIndex=%d AND GUID=%d", memo_index, guid);
			this->m_MailDB.WriteBlob(szMsg, (LPBYTE)lpMemoSendHdr->Subject, 60);

			ZeroMemory(szMsg, 1500);
			wsprintf(szMsg, "UPDATE T_FriendMail SET Memo=? where MemoIndex=%d AND GUID=%d", memo_index, guid);
			this->m_MailDB.WriteBlob(szMsg, (LPBYTE)sMemo, memo_size);

			return memo_index;
		}
		else
		{
			g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_WriteMail no result returned.");
			this->m_MailDB.Clear();
		}
	}
	else
	{
		g_Window.ServerLogAdd(ST_EXDATASERVER, "Error executing WZ_WriteMail");
		this->m_MailDB.Clear();
	}
...
void CFriendSystem::FriendMemoSend( int aIndex, FHP_FRIEND_MEMO_SEND* lpMsg)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail Send Request] Name[%s], ToName[%s], MailSize[%d], PhotoSize[%d].",
	...
	if(lpMsg->MemoSize <= 1000)
		...
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail Send Request] error: Mail Size > 1000.");
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail Send Request] Send Result [%d].", Result.Result);
	DataSend(aIndex, (LPBYTE)&Result, Result.h.size);
...
BOOL CFriendSystem::DBReadMail(int iMemoId, int iGuid, MEMO_READ * lpMemoRead, LPDWORD lpdwMemoSize, LPBYTE lpPhoto, LPBYTE lpPhotoSize)
{
	if(this->m_MailDB.ExecQuery("SELECT bRead, Dir, Act FROM T_FriendMail where MemoIndex=%d AND GUID =%d", iMemoId, iGuid))
	{
		...
		if(this->m_MailDB.Fetch() != SQL_NO_DATA)
		{
			bRead = this->m_MailDB.GetInt("bRead");
			lpMemoRead->Dir = this->m_MailDB.GetInt("Dir");
			lpMemoRead->Action = this->m_MailDB.GetInt("Act");
		}
		...
		*lpPhotoSize = this->m_MailDB.ReadBlob(szMsg, lpPhoto);
		...
		*lpdwMemoSize = this->m_MailDB.ReadBlob(szMsg, (LPBYTE)lpMemoRead->Memo);
...
void CFriendSystem::FriendMemoReadReq( int aIndex, FHP_FRIEND_MEMO_RECV_REQ* lpMsg)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail Read Request] Name: [%s], MemoIndex: [%d].", szMaster, lpMsg->MemoIndex);
	int guid = -1;
	guid = GetFriendGuid(szMaster);
	if(guid < 0)
	{
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail Read Request] error: GUID = -1.");
		return;
	}
	...
	if(DBReadMail(lpMsg->MemoIndex, guid, &memo, &memo_size, Photo, &btPhotoSize))
	{
		...
		g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail Read Request] Send Mail: size[%d], photo size[%d].", memo_size, btPhotoSize);
	}
}
...
BOOL CFriendSystem::DBDelMail(char *szName, int iMemoId)
{
	...
	if(this->m_MailDB.ExecQuery("EXEC WZ_DelMail '%s', %d", szName, iMemoId))
	{
		if(this->m_MailDB.Fetch() != SQL_NO_DATA)
		{
			...
			if(res != 1)
			{
				g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_DelMail returned: [%d]", res);
				res = FALSE;
			}
			else
				res = TRUE;
			...
			g_Window.ServerLogAdd(ST_EXDATASERVER, "WZ_DelMail no result returned.");
	}
	else
		g_Window.ServerLogAdd(ST_EXDATASERVER, "Error executing WZ_DelMail");
...
void CFriendSystem::FriendMemoDelReq( int aIndex, FHP_FRIEND_MEMO_DEL_REQ* lpMsg)
{
	...
	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail Delete Request] Name[%s], MamoIndex[%d].", szName, lpMsg->MemoIndex);

	Result.Result = DBDelMail(szName, lpMsg->MemoIndex);

	g_Window.ServerLogAdd(ST_EXDATASERVER, "[Mail Delete Request] Send Result [%d].", Result.Result);
	DataSend(aIndex, (LPBYTE)&Result, Result.h.size);
}
