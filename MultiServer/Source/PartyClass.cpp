...
{
//#if(DEV_AUTO_PTGUILD)
//	int iResult = PartyMatchJoin(lpMsg);
//#else
	...
	//g_Window.LogAdd(Log::Debug, "iResult: %d", pMsg.iResult);
	DataSend(aIndex,(LPBYTE)&pMsg,sizeof(pMsg));
//#endif
}
...
void DGPartyMatchRequestAnswer(PARTYMATCH_GDREQ_REQUESTANSWER* lpData, int aIndex)//3/5
{
	char Query[256];
	// ----
