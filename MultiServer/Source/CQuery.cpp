...
#if(ENABLE_ADDCODE)
BOOL CQuery::ExecQuery(LPTSTR szSQL, ...)
{
	int c;
	...
	va_list pArguments;
	va_start(pArguments, szSQL);
	vsprintf(szStatement, szSQL, pArguments);
	va_end(pArguments);

	if( S_TYPE != ST_CASHSHOPSERVER )
	{
		g_Window.ServerLogAdd(S_TYPE,"%s", szStatement);
	}

	ret=SQLExecDirect(hStmt,(SQLCHAR *)szStatement,SQL_NTS);
	if ((ret != SQL_SUCCESS) && (ret != SQL_SUCCESS_WITH_INFO) && (ret != SQL_NO_DATA)) {
		PrintDiag();
		return FALSE;
	}
