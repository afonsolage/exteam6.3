...
void ExTeleport::Draw()
{
	if(!gInterface.CheckWindowEx(exWinTeleport))
	{
		return;
	}
...
void ExTeleport::GCWindows(PMSG_GC_TELEPORTWIN* aRecv)
{
	this->Init();
	...
	{
		gInterface.OpenWindowEx(exWinTeleport);
	}
}
// ----------------------------------------------------------------------------------------------

void ExTeleport::CGResult(BYTE num)
{
	...
	gInterface.CloseWindowEx(exWinTeleport);
	this->ClickPage = -1;
}
// ----------------------------------------------------------------------------------------------

void ExTeleport::Button(DWORD Event)
{
	if(!gInterface.CheckWindowEx(exWinTeleport))
	{
		return;
...
void ExTeleport::SpeedCalc(int number, int &countplus)
{
	if(number > 0)
	{
		countplus++;
...
char* ExTeleport::ActiveText(int status)
{
	if(status)
	{
